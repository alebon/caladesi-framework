...
import net.caladesiframework.orientdb.db._
import java.util.{UUID, Locale}
import sun.reflect.generics.reflectiveObjects.NotImplementedException
import com.orientechnologies.orient.core.db.document.ODatabaseDocumentTx
import net.caladesiframework.orientdb.db.OrientDbRemoteType
import net.caladesiframework.orientdb.db.OrientDbRemoteConfiguration
...
abstract class OrientGraphRepository[EntityType <: OrientGraphEntity]
  ...
  private def emitDb = configuration.getType match {
      case dbType: OrientDbRemoteType =>
        val remoteConfig = configuration.asInstanceOf[OrientDbRemoteConfiguration]

        OGraphDatabasePool.global()
      ...
      case dbType: OrientDbEmbeddedType =>
        val localConfig = configuration.asInstanceOf[OrientDbEmbeddedConfiguration]

        // TODO: Refactoring to single initial call
        initLocalStore(localConfig.location)
        OGraphDatabasePool.global().acquire("local:%s".format(localConfig.location), "admin", "admin")
      case _ =>
  ...
  private def initLocalStore(path: String) = {
    try {
      ...
      case _ => throw new RuntimeException("Couldn't create DB at local store %s".format(path))
    }
